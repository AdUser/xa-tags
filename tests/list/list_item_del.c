...
int main()
{
  ...
  ret = list_item_add(data, "test2", 0);
  ret = list_item_add(data, "test3", 0);
  ret = list_item_add(data, "test4", 0);
  assert(data->len == 24);
  assert(memcmp(data->buf, "test1\0test2\0test3\0test4\0", 24) == 0);

  /** delete item from end */
  ret = list_item_del(data, "test4");
